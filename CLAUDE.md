...
`packages/mikro-orm-attachments` has a `bun test` suite (`test/`) exercising `AttachmentSubscriber`/`AttachmentConverter` against a real `flydrive` `FSDriver` on a temp dir, with fake `BaseConverter` subclasses standing in for `sharp`/`imgkit` — no live MikroORM/DB needed. Nothing else in the repo has tests, and there's still **no lint/check-types script** anywhere (the `apps/example` `test` script is a placeholder that exits 1; `turbo.json` declares a `check-types` task but no package implements it). Don't assume `bun run lint` does anything meaningful unless you add it yourself.
