...
3. **`AttachmentSubscriber`** (`src/subscribers/AttachmentSubscriber.ts`) is a MikroORM lifecycle subscriber with four hooks:
   ...
   - `afterUpdate`: for every updated entity, reads `changeSet.originalEntity` (MikroORM's pre-update snapshot) for each `Attachment` property that actually changed, reconstructs the *old* value via `AttachmentType.convertToJSValue`, and best-effort deletes its original + variant files from the resolved `Disk` — unless the new value happens to reuse the exact same key (possible with `rename: false`). Covers both "replaced with a new file" and "set to null".
   - `afterDelete`: for every hard-deleted entity (`em.remove()` + flush), enumerates its `Attachment` properties (via `Attachment[ATTACHMENT_FN_KEYS]`) and best-effort deletes the original + all variant files from the resolved `Disk`; storage errors are logged via `console.warn` and never abort the delete/update.
   - `afterUpdate` and `afterDelete` share a private `#deleteAttachmentFiles` helper for the actual disk-deletion + error-swallowing logic.
...
Storage cleanup on delete (`afterDelete`) and on replace (`afterUpdate`) only fires for entities changed through MikroORM's normal persist/remove lifecycle (`em.persist()`/`em.remove()` + flush). Soft-delete patterns (setting a `deletedAt` column via a plain update, e.g. `apps/example`'s `SoftDeletable` mixin) and raw `em.nativeUpdate()`/`em.nativeDelete()`/query-builder writes bypass entity hooks entirely — orphaned files are not cleaned up in those paths. This is an accepted limitation, not a bug.
